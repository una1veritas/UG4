...
def unit(llist,st,olist):
    ...
    if llist == []:
	    ...
	    print olist[ol-l]
	    if olist[ol-l][1] !=[]:
	        print "State:",l,olist[ol-l][1][0]
	        change(st,l,olist[ol-l][1][0])
	        print st
	    else:
	        if 
	        st = st+[[l]]
...
def check(string, current, work, accept):
    print string
    while string != "":
        letter = string[0]
	print
	print "Now:",current
        if (current,letter) in work: 
	    print "Next:",work[current,letter][0]
	    print "Accept?:", work[current,letter][1]
	    string = string[1:]
	    if string != "":
	        current = work[current,letter][0]
        else:
            return False
	    break
    else:
        return work[current,letter][1] in accept
